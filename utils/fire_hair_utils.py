...
class HairSegmentation():
    ...
    def __init__(self, webcam_width, webcam_height):
        ...
        # Read fire gif image
        self.num_fire_imgs, self.fire_imgs = get_fire_gif(
    ...
    def prepare_input(self, image):
        ...
        input_image = cv2.resize(
            input_image, (self.input_width, self.input_height))
    ...
    # def draw_dyed_hair(self, img, dyed_hair):
        # # Find the rectangle that surrounds all the contours
        # left, top, right, bottom = find_contours_rectangle()
    ...
    def draw_fire_hair(self, img, hair_mask):
        ...
        # fire_img = cv2.cvtColor(fire_img, cv2.COLOR_BGR2GRAY)
        fire_img[top:bottom, left:right] = cv2.resize(
            self.fire_imgs[self.fire_img_num][self.input_height//4:, :, :], (right-left, bottom-top))
...
def download_gdrive_model(gdrive_id, model_path):
    ...
    if not os.path.exists(model_path):
        ...
        shutil.move(
            "tmp/saved_model_512x512/model_float32_opt.onnx", model_path)
        shutil.rmtree("tmp/")
